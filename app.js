...
if(!state.reportDraft) state.reportDraft = {};
if(!state.reportTouched) state.reportTouched = {};
...
範例：
1. 行動性3分、思考性8分、慎重性9分，O同學目前較容易先思考、觀望與評估，面對升學、實習或就業選擇時，容易因目標過大而不知道如何開始。若O同學希望更清楚自己的職涯方向，建議透過實做來驗證，缺乏行動會使探索停留在想法中，難以累積實際判斷依據。

2. 持續性1分，O同學對長期任務的維持度較弱，偏好立竿見影的行動。目前較適合短期衝刺型的事務，建議可參加學校的微學分、演講等活動，來累積自己的經驗。在探索興趣的部分，每件事情至少要給自己1-3個月的時間累積足夠的經驗。`;

function switchTab(name){
  document.querySelectorAll(".tab").forEach(b => b.classList.toggle("active", b.dataset.tab === name));
  document.querySelectorAll(".panel").forEach(p => p.classList.toggle("active", p.id === name));
  if(name === "board") renderBoard();
  if(name === "report") renderReportGate();
}
...
function openDrawer(item, question){
  ...
  $("drawerSource").textContent = "O同學回答｜" + item.box;
...
function renderCpasGrid(id, arr){
  ...
  arr.forEach(item => {
    ...
    div.innerHTML = `<h4>${escapeHtml(item.title)}</h4>
      ...
      <p><strong>O同學回答：</strong>${escapeHtml(item.answer)}</p>
      <div class="purpose">${escapeHtml(item.purpose || "請依行為證據判讀。")}</div>`;
    grid.appendChild(div);
  });
}
$("unlockCpas").addEventListener("click", () => {
  state.cpasUnlocked = true; saveState(); renderCpas(); renderReportGate();
});
const goUnlockBtn = $("goUnlockCpas");
if(goUnlockBtn){
  goUnlockBtn.addEventListener("click", () => switchTab("cpas"));
}
...
const REPORT_FIELD_IDS = ["reportBackground","reportStatus","reportProblem","reportTraits12","reportWork5","reportLeadership","reportAdvice","reportImprovement"];

function reportDefaults(){
  const defaults = {};
  defaults.reportBackground = linesFrom(["背景與限制","學業與科系","經驗素材"]);
  defaults.reportStatus = linesFrom(["現況與卡點","升學/實習/就業","職缺與能力缺口"]);
  defaults.reportProblem = linesFrom(["主訴與價值觀","職涯問題線索"]);
  if(state.cpasUnlocked){
    defaults.reportTraits12 = REPORT_TRAITS_12;
    defaults.reportWork5 = `定型工作 4分：可接受一定程度的規則、流程與明確任務，但不一定能從高度重複工作中獲得強烈成就感。
...
若要探索創意或數位相關方向，可先從模仿、整理、改良、輔助型任務開始，例如簡報整理、資料視覺化、side project、小型數位工具作品。`;
    defaults.reportLeadership = `領導潛能 0分：O同學目前求學狀況較為迷失方向，面對責任容易選擇逃避。請從自我掌控、責任承擔、主動設定方向與推動任務的狀態撰寫，不建議從能不能當主管的角度分析。`;
  }else{
    // 尚未揭示 CPAS 時，CPAS 相關欄位保持空白，避免提前暴露分數與判讀。
    defaults.reportTraits12 = "";
    defaults.reportWork5 = "";
    defaults.reportLeadership = "";
  }
  // 顧問建議區由學員依職涯問題自行撰寫；提示文字放在驚嘆號與問號按鍵中。
  defaults.reportAdvice = "";
  defaults.reportImprovement = "";
  return defaults;
}
...
function buildReport(){
  ...
  REPORT_FIELD_IDS.forEach(id => {
    const el = $(id);
    if(!el) return;
    // 尚未手動修改的欄位，依目前蒐證資料自動帶入；已手動修改的欄位，保留學員文字。
    const value = state.reportTouched[id] ? (state.reportDraft[id] || "") : (defaults[id] || "");
    el.value = value;
  });
}

function setupReportAutosave(){
  REPORT_FIELD_IDS.forEach(id => {
    const el = $(id);
    if(!el || el.dataset.autosaveAttached === "1") return;
    el.dataset.autosaveAttached = "1";
    el.addEventListener("input", () => {
      state.reportTouched[id] = true;
      state.reportDraft[id] = el.value;
      saveState();
    });
  });
}
function collectReportText(){
  return `一、O同學背景\n${$("reportBackground").value}\n\n二、現況說明\n${$("reportStatus").value}\n\n三、職涯問題\n${$("reportProblem").value}\n\n四、12項人格特質說明\n${$("reportTraits12").value}\n\n五、五大適性工作\n${$("reportWork5").value}\n\n六、領導潛能\n${$("reportLeadership").value}\n\n七、顧問建議\n${$("reportAdvice").value}\n\n七、顧問建議---對O同學成為更好的具體改善建議\n${$("reportImprovement").value}`;
}
...
function reportHtml(){
  const sections = [
    ["一、O同學背景", $("reportBackground").value],
